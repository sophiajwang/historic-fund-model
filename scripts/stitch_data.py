...
Steps 3.1-3.4: Validate source files, join on (sector, domain, year),
compute derived metrics, and produce master file.

Output:
- data/unified/{sector}-unified.json
...
CPI_DEFLATORS = {
    ...
    2023: 1.000, 2024: 1.029, 2025: 1.050, 2026: 1.070  # 2024-2026 estimated
}
...
def validate_source_files(valid_domains):
    """Step 3.1: Validate source files against domain CSVs."""
    print("\nStep 3.1: Validating source files...")

    issues = []

    for sector in SECTORS:
        for source_type in ["private", "public", "government"]:
            filepath = SOURCE_DIR / f"{sector}-{source_type}.json"
            ...
            for record in data:
                ...
                if domain and domain not in valid_domains[sector]:
                    issues.append(f"{filepath.name}: Invalid domain '{domain}'")
    ...
    if issues:
        ...
        print("  All source files validated successfully")

    return len(issues) == 0


def load_source_data():
    """Load all source data into dictionaries keyed by (domain, year)."""
        ...
        for source_type in ["private", "public", "government"]:
            ...
            if filepath.exists():
                ...
                for record in records:
                    ...
                    data[sector][source_type][key] = record
...
def compute_derived_metrics(unified_records):
    """Step 3.3: Compute derived metrics for each unified record."""
...
def stitch_sector(sector, data):
    """Step 3.2: Join sources for a single sector."""
    unified = []
...
def main():
    print("Pipeline 3: Stitching")
    print("=" * 60)

    # Step 3.1: Validate
    valid_domains = load_valid_domains()
    if not validate_source_files(valid_domains):
    ...
    # Step 3.2: Join
    print("\nStep 3.2: Joining on (sector, domain, year)...")
    unified_data = {}
    ...
    # Step 3.3: Compute derived metrics
    print("\nStep 3.3: Computing derived metrics...")
    for sector in SECTORS:
        unified_data[sector] = compute_derived_metrics(unified_data[sector])
    print("  Done")

    # Step 3.4: Save output
    print("\nStep 3.4: Saving output files...")
    ...
    # Save master file
    master_path = MASTER_DIR / "all-sectors-unified.json"
